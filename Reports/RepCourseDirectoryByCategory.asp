...
QryStr = "SELECT * FROM Rep_CourseDirectory " & Filter & " ORDER BY Category"
...
<head>
    ...
    <title>Course Directory Report</title>
</head>

<body>
    <header>
        <h1 class="text-center">Training Management System</h1>
        <h3 class="text-center">Course Directory by Category</h3>
    </header>

    <%
        dim mCategory
        do while NOT RSCourseDirectory.EOF
            mCategory = RSCourseDirectory("Category")
            
    %>
    <main>
        <table class="table table-bordered" style="width: 95%;">
            <h4 class="text-center mt-2"><% response.write(mCategory) %></h4>
            <thead>
                <tr>
            ...
            <tbody>
                <%
                            Counter = 1
                            'response.write("mCategory = " & mCategory)
                            'response.write("<br>")
                            do while RSCourseDirectory("Category") = (mCategory)
                                
                            'response.write("RSCategory = " & RSCourseDirectory("Category"))
                            'response.write("<br>")
                %>
                <tr>
            ...
            <tfoot>
                <tr>
                    <td colspan="11">
                        <h5>Total Course Directories in Category (<% response.Write(mCategory) %>) :
