...
if Session("SUserRoleId") <> 2 then
    ...
    'Validations
        ...
        if ErrorFound=true or IsRoomConflict() or IsInstructorConflict() then
            response.Redirect("CourseDirectoryAdd.asp")
        end if
    'end
...
Function IsRoomConflict()
    ...
    QryStr = "SELECT * FROM V_CourseDirectoryView WHERE (StartDate <= '" & mEndDate & "') AND (EndDate >= '" & mStartDate & "') AND (CourseDirectoryStatusId = 1) AND (TimeSlotId IN " & GetConflictedTimeSlot() & ") AND (RoomId = " & mRoomId & ")"
    'response.write(QryStr)
...
Function IsInstructorConflict()
    ...
    Set RSFilterRec = Server.CreateObject("ADODB.RecordSet")
    QryStr = "SELECT * FROM V_CourseDirectoryView WHERE (StartDate <= '" & mEndDate & "') AND (EndDate >= '" & mStartDate & "') AND (CourseDirectoryStatusId = 1) AND (TimeSlotId IN " & GetConflictedTimeSlot() & ") AND (InstructorId = " & mInstructorId & ")"
    'response.write(QryStr)
    RSFilterRec.open QryStr, conn
